...
#SBATCH --nodes=1
module load SAMtools
file_array=($(ls fastq/*.fq))            #<2>
ind=$((SLURM_ARRAY_TASK_ID-1))     #<3>
current_file = $file_array[$ind]   #<4>
samtools view -c $current_file > "${current_file%sam}.txt" #<5>
```
1. Initialize job array to range from 1 to 10
2. list all files in `fastq/` with extension `.fq`. Assign it to the array `$file_array`
...
5. Run `samtools view -c` on `$current_file`
