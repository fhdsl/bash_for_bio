...
If I run `pwd` right after signing into `rhino`
...
One of the things we can do is check for disk usage with the `du` command. If I run `du` by itself on the command line, it will give me the disk usage of all folders and files in our current directory.
...
Navigate into the `bash_for_bio` repository. 
...
So, how do we utilize these filesystems? We first have to familiarize ourselves with *absolute* vs *relative* paths.

## Absolute versus relative paths {#sec-paths}

You may be wondering what the deal is, and whether it's necessary to know this. When you are processing files, it is important to understand the difference.
...
For the rest of the exercises, we'll be grabbing the scripts from github using `git clone`. 
