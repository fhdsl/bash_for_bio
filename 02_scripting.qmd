...
Say we want to see if `samtools` is installed on our HPC. One of the key commands you can use to find software is the `which` command.
...
If we don't have `samtools` immediately available, how do we find it on our system? We can use environment modules to load software.
