...
```
For more info, refer to Bite Sized Bash, page 6. 
...
REF_GENOME="/shared/biodata/reference/iGenomes/Homo_sapiens/UCSC/hg19/Sequence/BWAIndex/genome.fa" #<2>
bwa mem ${REF_GENOME} $1 > $1.aligned.sam #<3?>
module purge
```
1. Load `BWA` module.
2. Set a variable called `$REF_GENOME`.
3. Run `bwa mem` on the first argument `$1`, using the `$REF_GENOME` variable, and output to `$1.aligned.sam`.
...
csv_file <- read.csv(file=args$CSVFILE) #<1>
...
```
1. Use `args$CSVFILE` as an input for `read.csv()`
2. Use `args$CSVFILE` to specify the output name. 
...
We'll work on batch processing, both on a local system and the HPC cluster.
