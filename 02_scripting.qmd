...
3. Run `bwa mem` on the `$input_fasq`, using the `$ref_fasta_local` variable, and output to `${base_file_name}.sam`.
...
In our bash script, `my_bash_script.sh`, we're using positional argument (for simplicity) to specify our csvfile, and then passing the positional argument to named ones (`input_file`) for `my_r_script.R`.
