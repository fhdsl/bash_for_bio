...
One approach is to have your scripts also live in your project folder in `fast`. Then, you can sync the project in `/fh/fast/` over to `/hpc/temp/`, run the scripts in `/hpc/temp/`, and then sync the two folders again. You can do the file sync'ing in both directions with [Motuz](https://sciwiki.fredhutch.org/compdemos/motuz/), which has its own advantages. 
...
for file in (cat manifest.txt)  #<1>
IFS=""                #<2>
do
  read1=$(echo $file | awk '{print $1}')   #<3>
  read2=$(echo $file | awk '{print $2}')   #<4>
  bwa mem ${FASTA_INDEX_LOCATION} ${read1} ${read2} > ${OUTPUT_FOLDER}/${read1}.sam 
done

unset IFS             #<5>
```
1. Cycle through each line in `manifest.txt`
2. Change IFS to be `""` (no space), to process a file line by line.
3. Grab the word in the 1st position and assign it to `$read1`
4. Grab the word in the 2nd position and assign it to `$read2`
5. Reset IFS to original behavior.
...
Are instructions to `awk` to output the first word of our string. `awk` is an extremely powerful command, but we don't have enough time to cover it in full. Julia Evans has a great page on it in [Bite Size Command Line](https://wizardzines.com/comics/awk/).
...
## Removing file extensions with variable expansion
